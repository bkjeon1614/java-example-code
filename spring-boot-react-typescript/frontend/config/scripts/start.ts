...
export default async function start(config: StartConfiguration) {
  ...
  try {
    clientPort = await choosePort(HOST, DEFAULT_CLIENT_PORT)
    // serverPort = await choosePort(HOST, DEFAULT_SERVER_PORT)
  ...
  process.env.REACT_APP_CLIENT_PORT = clientPort
  ...
  const clientServer = clientApp.listen(clientPort, err => {
    ...
    console.log(chalk.cyan(`Starting the client on port ${clientPort}...\n`))
